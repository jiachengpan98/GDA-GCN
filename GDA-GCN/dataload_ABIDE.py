...
class dataloader():
    ...
    def load_data(self, connectivity='correlation', atlas1='aal',atlas2='ho'):
        ...
        fiqs = Reader.get_subject_score(subject_IDs, score='FIQ')
        ...
        fiq = np.zeros([num_nodes], dtype=np.float)
        for i in range(num_nodes):
            ...
            if fiqs[subject_IDs[i]]=='':
                continue
            else:
                fiq[i] = fiqs[subject_IDs[i]]
        ...
        phonetic_data = np.zeros([num_nodes, 5], dtype=np.float32)
        ...
        self.pd_dict['FIQ'] = np.copy(phonetic_data[:, 4])
    ...
    def get_PAE_inputs(self, nonimg):
        ...
        aff_adj,pd_affinity = Reader.get_static_affinity_adj(self.node_ftr1, self.pd_dict)
        flatten_ind = 0
        for i in range(n):
        ...
        return edge_index, edgenet_input,aff_adj,pd_affinity
