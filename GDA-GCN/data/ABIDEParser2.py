...
import torch
from utils.gcn_utils import normalize_features,sparse_mx_to_torch_sparse_tensor,normalize_adj,normalize
...
root_folder = '/media/pjc/datasets/ABIDE'
data_folder = os.path.join(root_folder, 'ABIDE_pcp/cpac/filt_noglobal')
phenotype = os.path.join(root_folder, 'ABIDE_pcp/Phenotypic_V1_0b_preprocessed1.csv')
data_folder2 = '/media/pjc/datasets/ABIDE/ABIDE_pcp_ho/filt_noglobal'
...
def get_ids(num_subjects=None):
    ...
    # subject_IDs = np.genfromtxt(os.path.join(data_folder, 'fewshot_subject_IDs.txt'), dtype=str)
...
def get_static_affinity_adj(features, pd_dict):
    pd_affinity = create_affinity_graph_from_scores(['SITE_ID','SEX'], pd_dict)
    #,'DSM_IV_TR'.AGE_AT_SCAN,SEX  'DSM_IV_TR' 'AGE_AT_SCAN','SEX'
    # pd_affinity=torch.from_numpy(pd_affinity)
    # pd_affinity=F.normalize(pd_affinity)
